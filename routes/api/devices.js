...
router.put('/:id/check', auth, async (req, res) => {
    ...
    var nextCheck = new Date(lastCheck.getFullYear(), lastCheck.getMonth() + device.period, lastCheck.getDate());
...
router.delete('/:id/check/:check_id', auth, async (req, res) => {
    ...
    // Get remove index
    const removeIndex = device.check.map(item => item.id).indexOf(req.params.check_id);
